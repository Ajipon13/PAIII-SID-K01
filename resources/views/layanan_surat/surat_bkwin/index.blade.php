...
<section class="content">
   ...
   <div class="row">
      <div class="col-6">
         @if(auth()->user()->level ==1)
         <a href="{{ url('sbk/create') }}" class=" mt-1 ml-2 btn btn border border-success shadow btn-outline-success"><i class="nav-icon fa fa-envelope" aria-hidden="true"></i> Buat Surat</a>
      </div>
      <div class="col-3">
         <form class="form-inline  mt-1"  action="{{url('sbk')}}" method="GET">
            ...
            <table class="table table-bordered">
               ...
               @foreach($data as $item)
                  ...
                  @if ($item->approved)
                     ...
                     <form action="/sbk/{{ $item->id }}/approve" method="post">
                     @csrf
                     @method('put')
                 ...
                 @endif
                  ...
                  @if($item->approved)
                  @if(auth()->user()->level == 1)
                        <a href="{{ url('menikah',$item->id) }} " class="text-warning btn-sm card bg-light mb-3 border border-warning" target="_blank" >Print</a>
                  @endif
                  @if(auth()->user()->level == 2)
                        <a href="{{ url('menikah2',$item->id) }} " class="text-warning btn-sm card bg-light mb-3 border border-warning" target="_blank" >Print</a>
                  @endif
                  @endif
                     </td>
                  </tr>
