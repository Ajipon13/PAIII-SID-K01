...
<section class="content">
   ...
   <div class="row">
      <div class="col-6">
         @if(auth()->user()->level ==1)
         <a href="{{ url('ktp/create') }}" class=" mt-1 ml-2 btn btn border border-success shadow btn-outline-success"><i class="nav-icon fa fa-envelope" aria-hidden="true"></i> Buat Surat</a>
      </div>
      <div class="col-3">
         <form class="form-inline  mt-1"  action="{{url('ktp')}}" method="GET">
      ...
      <div class="col-6">
         <form class="form-inline  mt-1"  action="{{url('surat_ktp')}}" method="GET">
            <div class="input-group input-group">
               <input class="form-control form-control-navbar shadow rounded" type="search" name="cari" placeholder="Cari..." aria-label="Search">
               <div class="input-group-append">
                  <button class="btn btn-navbar shadow rounded-circle text-dark"  style="background-color:#81BC00;" type="submit">
               ...
               @foreach($ktp as $item)
                  ...
                  @if ($item->approved)
                     @else
                        ...
                        <form action="/ktp/{{ $item->id }}/approve" method="post">
                        @csrf
                        @method('put')
                 ...
                 @endif
                  ...
                  @if($item->approved)
                     @if(auth()->user()->level == 1)
                        <a href="{{ url('pdfktp',$item->id) }} " class="text-warning btn-sm card bg-light mb-3 border border-warning" target="_blank" >Print</a>
                     @endif
                     @if(auth()->user()->level == 2)
                           <a href="{{ url('sktp',$item->id) }} " class="text-warning btn-sm card bg-light mb-3 border border-warning" target="_blank" >Print</a>
                     @endif
                  @endif
                     </td>
                  </tr>
